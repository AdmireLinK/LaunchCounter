...
func LoadConfig(filename string, config *Config) {
	...
	// 检查 JWT 密钥是否为空
	// 如果 JWT 密钥为空字符串，则需要生成一个新的密钥
	if config.JWTSecretKey == "" {
		// 生成一个 32 字节的随机密钥
		// 使用 crypto/rand 包生成安全的随机字节序列
		key := make([]byte, 32)
...
// 该函数会返回一个映射，键为用户 ID 格式化后的字符串，值为该用户对应的客户端信息列表。
// 每个客户端信息包含 IP 地址、连接时间和连接时长。
func GetOnlineClients() map[string][]map[string]interface{} {
	// 加读锁，防止在读取 Clients 时被其他协程修改
	ClientsLock.RLock()
	// 函数结束时释放读锁
	...
	for userID, clients := range Clients {
		...
		for _, client := range clients {
			...
			userClients = append(userClients, map[string]interface{}{
				"ip":         client.IP,
				// 格式化连接时间为 "2006-01-02 15:04:05" 格式
...
func (c *Client) WritePump() {
	// 使用 defer 确保在函数退出时关闭 WebSocket 连接
	defer func() {
		c.Conn.Close()
	}()
	...
	for {
		...
		case data, ok := <-c.Send:
			// 检查通道是否已关闭
			if !ok {
				// 通道已关闭，发送 WebSocket 关闭消息
				c.Conn.WriteMessage(websocket.CloseMessage, []byte{})
				return
			}

			// 序列化数据
			// 将接收到的数据使用 json.Marshal 方法转换为 JSON 字节切片
			jsonData, err := json.Marshal(data)
			if err != nil {
				// 序列化失败，记录错误日志并继续下一次循环
				log.Printf("序列化数据失败: %v", err)
				continue
			}

			// 发送JSON数据
			// 使用 WriteMessage 方法将 JSON 数据以文本消息的形式通过 WebSocket 连接发送
...
// 当读取过程中出现错误或者连接关闭时，会关闭 WebSocket 连接。
// 由于本项目不需要处理来自客户端的消息，仅做错误处理。
func (c *Client) ReadPump() {
	...
	for {
		...
		// 第一个返回值是消息类型，第二个是消息数据，第三个是可能的错误。
